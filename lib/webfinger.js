    ...
    getProperty = function(obj, Property) {
        ...
        if (Property.hasOwnProperty("$")) {
            ...
            if (Property.$.hasOwnProperty("xsi:nil") && Property.$["xsi:nil"] === "true") {
                obj[k] = null;
            } else if (Property.hasOwnProperty("_")) {
                obj[k] = Property._;
            } else {
                // TODO: log this
            }
    ...
    getLink = function(Link) {
        ...
        if (Link.hasOwnProperty("Property")) { // groan
            ...
            if (Array.isArray(Link.Property)) {
                for (i = 0; i < Link.Property.length; i += 1) {
        ...
        if (Link.hasOwnProperty("Title")) { 
            ...
            if (Array.isArray(Link.Title)) {
                for (i = 0; i < Link.Title.length; i += 1) {
    ...
    Step(
        ...
        function(err, doc) {
            ...
            if (err) {
                ...
                if (xrd.hasOwnProperty("Subject")) {
                    if (Array.isArray(xrd.Subject)) {
                        jrd.subject = xrd.Subject[0];
                    } else {
                        jrd.subject = xrd.Subject;
                    }
                }
                if (xrd.hasOwnProperty("Expires")) {
                    if (Array.isArray(xrd.Expires)) {
                        jrd.expires = xrd.Expires[0];
                    } else {
                        jrd.expires = xrd.Expires;
                    }
                }
                if (xrd.hasOwnProperty("Alias")) {
                    if (Array.isArray(xrd.Alias)) {
                        jrd.aliases = xrd.Alias;
                    } else {
                        jrd.aliases = [xrd.Alias];
                    }
                }
                if (xrd.hasOwnProperty("Property")) {
                    xrd.properties = {};
                    if (Array.isArray(xrd.Property)) {
                        for (i = 0; i < xrd.Property.length; i += 1) {
                            getProperty(jrd.properties, xrd.Property[i]);
                        }
                    } else {
                        getProperty(jrd.properties, xrd.Property);
                    }
                }
                if (xrd.hasOwnProperty("Link")) {
                    Link = xrd.Link;
                    jrd.links = [];
    ...
    Step(
        ...
        function(err, jrd) {
            ...
            if (options.webfingerOnly) {
                callback(new Error("Unable to find webfinger"), null);
                return;
            }
